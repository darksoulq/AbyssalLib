package com.github.darksoulq.abyssallib.common.serialization.internal.block_data.types.unique;

import com.github.darksoulq.abyssallib.common.serialization.Codec;
import com.github.darksoulq.abyssallib.common.serialization.Codecs;
import com.github.darksoulq.abyssallib.common.serialization.DynamicOps;
import com.github.darksoulq.abyssallib.common.serialization.internal.block_data.Adapter;
import org.bukkit.block.data.BlockData;
import org.bukkit.block.data.type.Farmland;

public class FarmlandAdapter extends Adapter<Farmland> {
    @Override
    public boolean doesApply(BlockData data) {
        return data instanceof Farmland;
    }

    @Override
    public <D> D serialize(DynamicOps<D> ops, Farmland value) throws Codec.CodecException {
        return Codecs.INT.encode(ops, value.getMoisture());
    }

    @Override
    public <D> void deserialize(DynamicOps<D> ops, D input, BlockData base) throws Codec.CodecException {
        if (!(base instanceof Farmland farmland)) return;
        int value = Codecs.INT.decode(ops, input);
        if (value > farmland.getMaximumMoisture()) return;
        farmland.setMoisture(value);
    }
}
