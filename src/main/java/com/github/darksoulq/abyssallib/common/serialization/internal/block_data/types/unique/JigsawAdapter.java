...
public class JigsawAdapter extends Adapter<Jigsaw> {
    ...
    @Override
    public <D> DataResult<D> serialize(DynamicOps<D> ops, Jigsaw value) {
        return CODEC.encode(ops, value.getOrientation());
    }

    @Override
    public <D> DataResult<Void> deserialize(DynamicOps<D> ops, D input, BlockData base) {
        if (!(base instanceof Jigsaw jigsaw))
            return DataResult.error(DataError.custom("Base is not Jigsaw, got: " + base.getClass().getSimpleName()));
